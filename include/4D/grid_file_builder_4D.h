...
// #include "MLP.h"
#include "common.h"
#include "grid_file_4D.h"
#include "grid_file_builder_3D.h"
...
namespace gf4D
{
  ...
  private:
    ...
    void DoFitting()
    {
      ...
      int group_size_3d_ = ceil(group_size_ / pow(group_size_ / cell_size_, 1.0 / 3));
      gf3D::Builder<uint64_t> gfb3d(group_points_.front()[0], group_points_.back()[0], group_size_3d_, rs_group_size_, num_radix_bits_, cell_size_);
      for (size_t i = 0; i < n; i++)
      {
        if ((i + 1) % group_size_3d_ == 0 || i == n - 1)
        {
          gfb3d.AddKeyToSpline(group_points_[i][0]);
        }
        vector<uint64_t> tmp = {group_points_[i][1],group_points_[i][2]};
        gfb3d.AddKey(tmp);
      }
      gf3D::GridFile<uint64_t> gf3d = gfb3d.Finalize();
      gf3ds_.push_back(gf3d);
    }
